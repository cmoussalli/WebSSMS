...
A side panel that writes T-SQL for you. Describe what you want — "the ten customers with the highest
order value this year", "a procedure that archives orders older than two years" — and the script is
appended to the bottom of the query tab you have open, under a comment saying which model wrote it and
what was asked for.

It is not a chatbot guessing at your schema. Before it writes anything it reads the live database
through a fixed set of **read-only** lookups: list tables, describe a table's columns and keys, read a
view or procedure definition, and sample rows with a single `SELECT`. If the request is still ambiguous
after looking — which of two similar tables, what "active" means here, whether a removal is a `DELETE`
or a flag — it comes back with numbered questions instead of a script.

Point it at **Ollama** on your own machine, **DeepSeek**, **OpenAI**, or anything else that speaks the
OpenAI chat-completions protocol. See [AI assistant](#ai-assistant) for setup and for what it can and
cannot do to your server.
...
Open it from the **AI** button in the toolbar, or **AI Assistant** in the menu bar. Type what you want
in plain English and press **Generate** (or `Ctrl+Enter`).

### What happens when you ask

1. **It looks at your database.** The model is given read-only functions and told to use them before it
   writes: `list_databases`, `list_tables`, `describe_table`, `list_routines`,
   `get_object_definition`, and `run_select` for sampling data. Every lookup it made is listed above
   the answer — expand *Checked N things in the database* to see exactly what it read.
2. **It asks if the request is ambiguous.** When two tables would both fit, when a word like "recent"
   has no obvious meaning in your schema, or when a removal could be a `DELETE` or a soft-delete flag,
   the reply is a short list of numbered questions and no script. Answer in the same panel and it
   continues.
3. **The script lands in your query tab.** Nothing is executed. The script is appended to the end of the
   tab you have open — a new tab is created if there is none — under a header comment:

   ```sql
   -- ============================================================================
   -- Generated by the WebSSMS AI assistant -- review before running.
   -- Model:   gpt-oss:20b (Ollama (local))
   -- Created: 2026-08-20 11:04:32
   -- Request: list the ten customers with the highest total order value this year
   -- ============================================================================
   ```

   **Insert again** re-appends it; **Copy** puts it on the clipboard with the same header.

The conversation is kept for follow-ups ("now add an index for that query"), and the text already in
your tab is sent as context so "the query above" means something. **New conversation** clears both.

### Choosing a provider

Open **AI settings** from the panel header.

| Provider | Endpoint | Key | Notes |
| --- | --- | --- | --- |
| **Ollama** | `http://localhost:11434/v1` | none | Runs on your machine; nothing leaves it |
| **DeepSeek** | `https://api.deepseek.com/v1` | required | |
| **OpenAI** | `https://api.openai.com/v1` | required | |
| **Custom** | whatever you enter | optional | vLLM, LM Studio, OpenRouter, a corporate gateway |

All four are spoken to as OpenAI `/chat/completions`, so "Custom" covers anything else that implements
it. **Test connection** asks the endpoint for its model list and tells you whether the model you named
is among them.

**Pick a model that can call tools.** Schema lookups are function calls; a model without tool support
will ignore them and invent tables. For local models that cannot, turn *Look up schema with tools* off:
the assistant then gets a one-shot digest of the current database's tables and columns instead of live
lookups, and is told to ask you about anything the digest does not cover.

Other settings worth knowing:

- **Let it read rows** — allows `run_select`. Off, it works from structure alone.
- **Rows per lookup** (default 20) and **Lookup limit** (default 12) bound how much it reads per
  question.
- **Temperature** defaults to `0.1`, which keeps generated SQL predictable. Set **Max tokens** to `0`
  or temperature to `-1` to leave the field out of the request — a few models reject them.

**Save** writes the settings to `App_Data/ai-settings.json` on the server, so they are still there the
next time the app starts and in any browser you open it from. Clear *Store the API key with the settings*
to keep the key in memory until the app restarts — everything else is saved either way. Point
`Ai:SettingsFile` somewhere else to move the file; in Docker, mount that path if the config should
outlive the container. The file holds the key in plain text, so on a shared deployment leave the key out
of it and supply it through `appsettings.json` or an environment variable instead.

**Server default** deletes the saved file and falls back to `appsettings.json`:

```jsonc
// appsettings.json
"Ai": {
  "Enabled": true,          // false removes the panel from the UI entirely
  "SettingsFile": "",       // where Save writes; empty = App_Data/ai-settings.json
  "Provider": "Ollama",     // Ollama | DeepSeek | OpenAI | Custom
  "BaseUrl": "",            // empty = the provider default
  "Model": "",              // empty = the provider default
  "ApiKey": "",             // better left empty; enter it in the settings dialog
  "Temperature": 0.1,
  "MaxTokens": 4096,        // 0 = omit the field
  "TimeoutSeconds": 120,
  "UseTools": true,         // false = one-shot schema digest instead of live lookups
  "MaxToolCalls": 12,
...
### What it can and cannot do to your server

The assistant never runs the script it writes — that is yours to read and execute. Its own lookups are
read-only, and three separate things keep them that way:

- **Only six functions exist**, none of which takes a free-form batch. Structure lookups are
  parameterised queries against `sys.*`; a database name is checked against `sys.databases` before it is
  ever used.
- **Ad-hoc queries are parsed first.** `run_select` accepts a single statement that starts with `SELECT`
  or `WITH`. Comments and string literals are stripped before checking, so a hidden second statement or
  a keyword buried in a literal cannot slip through; anything that writes, executes or reaches outside
  the session — `INSERT`, `UPDATE`, `DELETE`, `DROP`, `EXEC`, `INTO`, `OPENROWSET`, `DBCC` … — is
  rejected.
- **Every query runs in a transaction that is rolled back**, whatever the parser let through.

It uses its own SQL connection, cloned from your credentials with the application name `WebSSMS AI`, so
you can see it in Activity Monitor and so its database hopping never disturbs the database your query
tab is pointed at. It has exactly the permissions of the login you connected with — give it a
least-privilege login if that matters to you.

**Schema and sampled rows are sent to the model.** With Ollama that is a local process; with a hosted
provider it leaves your machine. If your data is sensitive, use a local model, or turn *Let it read
rows* off so only structure is sent.
...
│   ├── Ai/               AI assistant panel and its settings dialog
...
| `SqlAiAgent` | Drives the LLM: prompt, tool loop, script extraction, conversation |
| `SqlAiToolbox` | The read-only lookups the model may call, on its own connection |
| `ReadOnlySqlGuard` | Rejects anything but a single `SELECT` from the assistant |
| `LlmClient` | OpenAI-compatible `/chat/completions` transport, with tool calling |
| `AiSettingsProvider` | The LLM settings in force for this circuit |
...
- **Backup transfers can read and write files on the server.** Downloads and uploads are gated by
  ...
  [Backup file transfer](#backup-file-transfer).
- **The AI assistant sends your schema — and, if enabled, sampled rows — to whatever endpoint you point
  it at.** With a local Ollama that stays on your machine; with a hosted provider it does not. Its own
  queries are read-only and rolled back, and it never executes what it writes, but it reads with the
  permissions of your login. Set `Ai:Enabled` to `false` to remove the feature entirely, or turn off
  *Let it read rows* to keep row data out of the prompt — see [AI assistant](#ai-assistant).
